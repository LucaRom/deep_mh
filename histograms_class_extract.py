...
e_mask_multi = os.path.join(pre_path, "00_Donnees/02_maitrise/01_trainings/estrie/processed_raw/mask/estrie_mask_multiclass_3m.tif")

#print(e_sen2_ete)

# # class counter (temp) # TODO move as an utils... (utils folder?)
# labels = (0, 1, 2, 3, 4, 5, 6, 7)
# labels_dict = dict.fromkeys(labels, 0)

# na = np.array(tiff.imread(e_mask_multi ))
...
#im_estrie = rasterio.open(e_sen2_ete)
#mask_estrie = rasterio.open(e_mask_multi)
...
k_sen2_ete = os.path.join(pre_path, "00_Donnees/02_maitrise/01_trainings/kenauk/raw_standard/s2_kenauk_3m_ete_HMe_STD.tif")
k_mask_multi = os.path.join(pre_path, "00_Donnees/02_maitrise/01_trainings/kenauk/processed_raw/mask/kenauk_mask_multiclass_3m.tif")

im_kenauk = rasterio.open(k_sen2_ete)
mask_kenauk = rasterio.open(k_mask_multi)

ar_e = im_kenauk.read(out_dtype='float32')
ar_mask_e = mask_kenauk.read(out_dtype='float32')

#combined_arrs = np.concatenate((ar_e, ar_mask_e))
...
for i in range(8):
    ...
    red_e_filter   = copy.copy(red_e)
    green_e_filter = copy.copy(green_e)
    blue_e_filter  = copy.copy(blue_e)
    nir_e_filter   = copy.copy(nir_e)


    red_e_filter[ar_mask_e[0] != selected_class]   = np.nan
    green_e_filter[ar_mask_e[0] != selected_class] = np.nan
    blue_e_filter[ar_mask_e[0] != selected_class]  = np.nan
    nir_e_filter[ar_mask_e[0] != selected_class]   = np.nan

    kwargs = dict(alpha=0.5, bins=255)

    # TODO Fix weights values by removing nan from len()
    plt.hist(red_e_filter.flatten(), **kwargs, color='r', label='Red', weights=np.ones_like(red_e_filter.flatten()) / len(red_e_filter.flatten()), histtype="step")
    plt.hist(green_e_filter.flatten(), **kwargs, color='g', label='Green', weights=np.ones_like(green_e_filter.flatten()) / len(green_e_filter.flatten()), histtype="step")
    plt.hist(blue_e_filter.flatten(), **kwargs, color='b', label='Blue', weights=np.ones_like(blue_e_filter.flatten()) / len(blue_e_filter.flatten()), histtype="step")
    plt.hist(nir_e_filter.flatten(), **kwargs, color='y', label='NIR', weights=np.ones_like(nir_e_filter.flatten()) / len(nir_e_filter.flatten()), histtype="step")

    plt.legend()
    plt.suptitle('Kenauk - '+ 'Class no : ' + str(i))
