...
from unet_mono import UNet_mono
...
class SemSegment(LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # TODO UNDERSTAND AND MAKE SELF (credit julien)
        preds = self(img)
        val_loss = torch.nn.CrossEntropyLoss()(preds, mask)

        y_pred = preds.softmax(dim=1).argmax(dim=1).unsqueeze(0)
        y_tgt = mask.unsqueeze(0)
        ...
        val_accu = self.val_accuracy(y_pred, y_tgt)
        val_f1   = self.val_f1(y_pred, y_tgt)
