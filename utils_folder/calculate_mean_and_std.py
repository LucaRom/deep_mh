...
def mean_std_solo(path_image):
    ...
    for band in array:
        ...
        if np.isnan(band_flat).any():
            print()
            print("There are NaN values in the array")
            print()
            mean = np.nanmean(band_flat)
            std = np.nanstd(band_flat)
        else:
...
if __name__ == "__main__":
    ...
    # #paths_lst = img_paths.get_portneuf_z2_raw_paths(output_mode='list')

    # lidar_means_array = []
    # lidar_stdev_array = []
    # for path in paths_lst:
    #     mean_lst, std_lst, num_bands = mean_std_solo(path_image=path)
    #     sensors_name = path.split("/")[-2] #(chemin estrie)
    ...
    # current_region = paths_lst[0].split("/")[-4]
    ...
    # # Solo mnt pour GMQ805
    # path_image= "/mnt/d/00_Donnees/02_maitrise/01_trainings/estrie/processed_raw/lidar/MNT_estrie_3m.tif"

    # allo = mean_std_solo(path_image)

    # print(allo)

    
    path_to_full_sen2_ete ="/mnt/d/00_Donnees/02_maitrise/01_trainings/portneuf_zone1/processed_raw/sen2_ete/s2_3m_portneuf_ete_nord_2018.tif"
    path_to_full_sen2_print = "/mnt/d/00_Donnees/02_maitrise/01_trainings/portneuf_zone1/processed_raw/sen2_pri/s2_3m_portneuf_pri_nord_2018.tif"
    path_to_full_sen1_ete = "/mnt/d/00_Donnees/02_maitrise/01_trainings/portneuf_zone1/processed_raw/sen1_ete/s1_3m_portneuf_ete_nord_2018.tif"
    path_to_full_sen1_print = "/mnt/d/00_Donnees/02_maitrise/01_trainings/portneuf_zone1/processed_raw/sen1_pri/s1_3m_portneuf_pri_nord_2018.tif"
    path_to_full_mhc = "/mnt/d/00_Donnees/02_maitrise/01_trainings/portneuf_zone1/processed_raw/lidar/mhc_3m_portneuf_nord.tif"
    path_to_full_slopes = "/mnt/d/00_Donnees/02_maitrise/01_trainings/portneuf_zone1/processed_raw/lidar/slo_3m_portneuf_nord.tif"
    path_to_full_tpi = "/mnt/d/00_Donnees/02_maitrise/01_trainings/portneuf_zone1/processed_raw/lidar/tpi_3m_portneuf_nord.tif"
    path_to_full_tri = "/mnt/d/00_Donnees/02_maitrise/01_trainings/portneuf_zone1/processed_raw/lidar/tri_3m_portneuf_nord.tif"
    path_to_full_twi = "/mnt/d/00_Donnees/02_maitrise/01_trainings/portneuf_zone1/processed_raw/lidar/twi_3m_portneuf_nord.tif"


    sen2 = [path_to_full_sen2_ete, path_to_full_sen2_print]
    sen1 = [path_to_full_sen1_ete, path_to_full_sen1_print] 
    lidar = [path_to_full_mhc, path_to_full_slopes, path_to_full_tpi, path_to_full_tri, path_to_full_twi]
    ...
    for path in lidar:
        mean_lst, std_lst, num_bands = mean_std_solo(path_image=path)
        for i in mean_lst:
            full_mean_lst.append(i)
        for i in std_lst:
            full_stdevs_lst.append(i)

    np.save('stats/portneuf_zone1/lidar_means_portneuf_nord', full_mean_lst)
    np.save('stats/portneuf_zone1/lidar_stdev_portneuf_nord', full_stdevs_lst)

    print(full_mean_lst)
    print(full_stdevs_lst)

    # allo =  np.load('stats/estrie/estrie_sen2_ete_means.npy')
    # print(allo)