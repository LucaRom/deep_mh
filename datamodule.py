...
class MultiSourceDataModule(LightningDataModule):
    def __init__(self, train_config_path, dataset_config_path=None, extra_test_mask_dirs=None):
        super().__init__()
        self.dataset_config = OmegaConf.load(dataset_config_path)
        self.train_config = train_config_path # usually cfg from Hydra
        #self.train_config = OmegaConf.load(train_config_path) # FOR DEBUG
        ...
        if dataset_config_path == 'conf/datasets/portneuf_sud.yaml':
            self.load_indexes_ps()
        elif dataset_config_path == 'conf/datasets/portneuf_nord.yaml':
            self.load_indexes_pn()
        elif dataset_config_path == 'conf/datasets/estrie.yaml':
            self.load_indexes_es()
        else:
            print(f'Unrecognized config_path value : {dataset_config_path}')
    ...
    def load_config_params(self):
        ...
        self.sample_factor = self.train_config.model.sample_factor
        self.test_mask_dir = self.train_config.model.test_mask_dir
    ...
    def load_indexes_pn(self):
        valid_idx = OmegaConf.to_container(self.dataset_config.indexes.idx_range.valid)
        not_256 = OmegaConf.to_container(self.dataset_config.indexes.idx_range.not256)
        ...
        valid_idx_tmp_set = set(valid_idx)
        not_256_set = set(not_256)
        test_idx_set = set(all_test_idx)

        valid_idx_set = [idx for idx in valid_idx_tmp_set if idx not in not_256_set]
    ...
    def load_indexes_ps(self):
        # ranges
        full_idx_range = OmegaConf.to_container(self.dataset_config.indexes.idx_range.full_idx)
        inv_top_range = OmegaConf.to_container(self.dataset_config.indexes.idx_range.inv_top)
        inv_bot_range = OmegaConf.to_container(self.dataset_config.indexes.idx_range.inv_bot)
        row1_range = OmegaConf.to_container(self.dataset_config.indexes.idx_range.row1)
        row2_range = OmegaConf.to_container(self.dataset_config.indexes.idx_range.row2)
        row3_range = OmegaConf.to_container(self.dataset_config.indexes.idx_range.row3)
        row4_range = OmegaConf.to_container(self.dataset_config.indexes.idx_range.row4)
        row5_range = OmegaConf.to_container(self.dataset_config.indexes.idx_range.row5)
        row6_range = OmegaConf.to_container(self.dataset_config.indexes.idx_range.row6)
        row7_range = OmegaConf.to_container(self.dataset_config.indexes.idx_range.row7)
        row8_range = OmegaConf.to_container(self.dataset_config.indexes.idx_range.row8)

        # lists
        not_256_list = OmegaConf.to_container(self.dataset_config.indexes.idx_range.not256)
        inv_left =  OmegaConf.to_container(self.dataset_config.indexes.idx_range.inv_left)
        inv_right =  OmegaConf.to_container(self.dataset_config.indexes.idx_range.inv_right)
        row_mix = OmegaConf.to_container(self.dataset_config.indexes.idx_range.row_mix)
        inv_mix = OmegaConf.to_container(self.dataset_config.indexes.idx_range.inv_mix)

        all_idx_tmp = list(range(full_idx_range[0], full_idx_range[1]))

        all_invalid_tmp = (list(range(inv_top_range[0], inv_top_range[1])) +
                            list(range(inv_bot_range[0], inv_bot_range[1])) +
                            not_256_list + inv_left + inv_right + inv_mix)

        all_test_idx_tmp = (list(range(row1_range[0], row1_range[1])) +
                            list(range(row2_range[0], row2_range[1])) +
                            list(range(row3_range[0], row3_range[1])) +
                            list(range(row4_range[0], row4_range[1])) +
                            list(range(row5_range[0], row5_range[1])) +
                            list(range(row6_range[0], row6_range[1])) +
                            list(range(row7_range[0], row7_range[1])) +
                            list(range(row8_range[0], row8_range[1])) +
                            row_mix)

        # Filter to include only those within the valid indexes and remove test idx from trainval idx
        all_idx_set = set(all_idx_tmp)
        all_invalid_set = set(all_invalid_tmp)
        all_test_idx_set = set(all_test_idx_tmp)

        self.test_idx = [idx for idx in all_test_idx_set]
        self.trainval_idx = list(all_idx_set - all_invalid_set - all_test_idx_set)

        # print("Train/Val indexes within valid range:", self.trainval_idx)
        # print("Test indexes within valid range:", self.test_idx)

    def load_indexes_es(self):
        full_idx = OmegaConf.to_container(self.dataset_config.indexes.idx_range.full_idx)
        not_256 = OmegaConf.to_container(self.dataset_config.indexes.idx_range.not256)
        test_idx_range = OmegaConf.to_container(self.dataset_config.indexes.idx_range.test)

        # Load list from range                
        all_idx = list(range(full_idx[0], full_idx[1]))

        # Convert lists to sets
        all_idx_set = set(all_idx)
        not_256_set = set(not_256)
        test_idx_set = set(test_idx_range)

        self.trainval_idx = list(all_idx_set - not_256_set - test_idx_set)
        self.test_idx = [idx for idx in test_idx_set]

        # print("Train/Val indexes within valid range:", self.trainval_idx)
        # print("Test indexes within valid range:", self.test_idx)

    def setup(self, stage=None):
        if self.test_mask_dir == 'mask_multiclass_3223_buff':
            main_mask_dir = os.path.join(self.paths['pre_path_tiled'], 'mask_multiclass_3223_buff')
        else:
        ...
        temp_dataset = MultiSourceDataset(
        ...
        #if self.train_config.mode not in ['test_ckpts', 'inference', 'test']:
        # Calculate background proportion for each tile
        # background_proportion = temp_dataset.calculate_background_proportion(main_mask_dir, self.trainval_idx)
        
        # # Define a threshold for filtering
        # background_threshold = 2

        # # Filter out tiles with a high proportion of background class
        # filtered_trainval_idx = [idx for idx in self.trainval_idx if background_proportion[idx] < background_threshold]
        filtered_trainval_idx = self.trainval_idx

        # Calculate class distribution and weights
        class_dist = temp_dataset.calculate_class_distribution(main_mask_dir, filtered_trainval_idx)
        ...
        train_class_weights = temp_dataset.calculate_class_weights(aggregated_counts)

        train_sample_weights = temp_dataset.calculate_sample_weights(train_idx, class_dist, train_class_weights)
        sampler = WeightedRandomSampler(train_sample_weights, num_samples=int(len(train_sample_weights)*self.sample_factor), replacement=True)

        # if self.train_config.model.classif_mode == 'bin':
        #     self.train_class_weights = temp_dataset.derive_binary_weights_from_multiclass(train_class_weights)
        # else:
        ...
        self.main_test_dataset = MultiSourceDataset(
            dataset_config=self.dataset_config,
            train_config=self.train_config,
            img_train_dir=self.paths['pre_path_tiled'],
            mask_dir=main_mask_dir,
        ...
        if self.extra_test_mask_dirs:
            self.additional_test_dataset = MultiSourceDataset(
                dataset_config=self.dataset_config,
                train_config=self.train_config,
                img_train_dir=self.paths['pre_path_tiled'],
    ...
    def test_dataloader(self):
        return DataLoader(self.main_test_dataset, batch_size=1, num_workers=1, pin_memory=False)
...
class MultiSourceDataset(Dataset):
    ...
    def calculate_background_proportion(self, mask_dir, idx_list, background_class=7):
        ...
        for idx in idx_list:
            ...
            # Calculate the proportion of the background class
            total_pixels = mask.size
            background_pixels = np.sum(mask == background_class)
            background_proportion[idx] = background_pixels / total_pixels
        
        return background_proportion
    ...
    def derive_binary_weights_from_multiclass(self, multiclass_weights):
        """
        Derive binary weights from multiclass weights by summing the appropriate class weights.

        Args:
        multiclass_weights (tensor): Tensor of multiclass weights.

        Returns:
        tensor: Tensor of binary weights.
        """
        binary_weights = torch.zeros(2)
        binary_weights[0] = multiclass_weights[7]  # Class 7 -> binary class 0
        binary_weights[1] = torch.sum(multiclass_weights) - multiclass_weights[7]  # All other classes -> binary class 1
    ...
    #     normalized_weights = {class_id: weight / weight_sum for class_id, weight in class_weights.items()}      
        
    #     sorted_class_weights = sorted(normalized_weights.items(), key=lambda item: item[0])
    ...
    def __getitem__(self, index):
        ...
        if len(self.indices_lst) > 0 and len(self.opt_bands) == 0:
            img_opt = torch.cat([*summer_indices_tensors, *spring_indices_tensors], dim=0)

        if self.transforms:
            img_opt, img_rad, img_lidar, mask = geo_transform(img_opt, img_rad, img_lidar, mask)
        
        if self.bin_feature:
            mask_binaire = np.where((mask != 7) & (mask != -1), 1, 0).astype(np.float32)
            return img_opt, img_lidar, mask_binaire, mask, img_rad, sen2_ete_path
        else:
            return img_opt, img_lidar, mask, img_rad, sen2_ete_path
    
if __name__ == "__main__":
    dataset_config_path = 'conf/datasets/estrie.yaml'
    train_config_path = 'conf/model/194_debug.yaml'

    module = MultiSourceDataModule(
